 ...
 */
class SeekBarCompat : SeekBar, View.OnTouchListener {
    ...
    private fun setupProgressColor() {
        try {
            ...
            val ninePatchDrawable = ld.findDrawableByLayerId(android.R.id.background) as NinePatchDrawable
    ...
    private fun setupProgressBackground() {
        //load up the drawable and apply color
        val seekBarBackgroundDrawable = SeekBarBackgroundDrawable(context,
                mProgressBackgroundColor, mActualBackgroundColor, paddingLeft.toFloat(), paddingRight.toFloat())
        if (belowJellybean())
    ...
    override fun setEnabled(enabled: Boolean) {
        ...
        triggerMethodOnceViewIsDisplayed(this, Callable<Void> {
            if (!lollipopAndAbove()) {
                ...
                val ld = progressDrawable as LayerDrawable
                val shape = ld.findDrawableByLayerId(android.R.id.progress) as ScaleDrawable
                shape.setColorFilter(if (mIsEnabled) mProgressColor else Color.LTGRAY, PorterDuff.Mode.SRC_IN)
                //set the background to transparent
                val ninePatchDrawable = ld.findDrawableByLayerId(android.R.id.background) as NinePatchDrawable
                ninePatchDrawable.setColorFilter(Color.TRANSPARENT, PorterDuff.Mode.SRC_IN)
                //background
                //load up the drawable and apply color
                val seekBarBackgroundDrawable = SeekBarBackgroundDrawable(context,
                        if (mIsEnabled) mProgressBackgroundColor else Color.LTGRAY, mActualBackgroundColor, paddingLeft.toFloat(), paddingRight.toFloat())
                if (belowJellybean())
                    setBackgroundDrawable(seekBarBackgroundDrawable)
                else
                    background = seekBarBackgroundDrawable
    ...
    companion object {
         ...
         */
        fun triggerMethodOnceViewIsDisplayed(view: View, method: Callable<Void>) {
